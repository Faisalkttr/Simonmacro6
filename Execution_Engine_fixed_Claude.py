...
def resolve_action():
    ...
    return ("SYSTEM PHASE", playbook["stance"],
...
def system_governor(target_alloc, kill_switch, discipline):
    if kill_switch:
        return target_alloc * 0.5, "Liquidity kill-switch active: suggested target halved as a precaution."
    if discipline == "VIOLATION":
        return target_alloc * 0.8, "Large deviation from your own reported allocation: suggested target trimmed rather than jumping straight to the full policy number."
...
rebalance_note = rebalance_suggestion(current_alloc, governed_target)
